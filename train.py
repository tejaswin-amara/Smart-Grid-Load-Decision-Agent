...
def train_agent() -> None:
    ...
    training_env = DummyVecEnv([lambda: AdvancedSmartGridEnv(config=config, horizon_hours=24)])
    training_env = VecNormalize(training_env, norm_obs=True, norm_reward=True, clip_obs=10.0)
    eval_env = DummyVecEnv([lambda: AdvancedSmartGridEnv(config=config, horizon_hours=24)])
