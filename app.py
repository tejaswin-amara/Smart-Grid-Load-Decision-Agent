...
@st.cache_resource
def load_trained_model(model_path: str) -> SAC:
    """Load trained SAC model from disk with caching."""
    if not os.path.exists(model_path):
        st.error(f"Model not found at {model_path}")
        st.info("Please run `python train.py` first to train the agent.")
        st.stop()
...
def run_evaluation_episode(
    ...
    """
    Run a deterministic evaluation episode with the trained agent.
...
def main():
    ...
    if run_simulation:
        ...
        with col3:
            st.success(f"⏸️ **Idle Periods**: {idle_periods} minutes")
